...
public class TariffMarketBean implements Serializable
{
                           ...
                           VisualizerHelperService helper)
  {
    ...
    for (Iterator iterator = brokers.iterator(); iterator.hasNext();) {
      ...
      Collection<TariffData> allTariffsData =
        brokerModel.getTariffCategory().getTariffData().values();
      for (TariffData td: allTariffsData) {
        if (td.getCustomers() > 0) {
          ...
          customerTypeSpecific
                  .put(td.getSpec().getPowerType(),
                       customerTypeSpecific.get(td.getSpec().getPowerType())
                               + td.getCustomers());
        }
      }

      ArrayList<Object> data_c = new ArrayList<Object>();
      data_c.add(new PowerTypeTemplate("Consumption", customerTypeSpecific
              .get(PowerType.CONSUMPTION)));
      data_c.add(new PowerTypeTemplate(
      ...
      data_p.add(new PowerTypeTemplate("Production", customerTypeSpecific
              .get(PowerType.PRODUCTION)));
      data_p.add(new PowerTypeTemplate("Chp production", customerTypeSpecific
              .get(PowerType.CHP_PRODUCTION)));
      data_p.add(new PowerTypeTemplate(
                                       "Fossil production",
                                       customerTypeSpecific
                                               .get(PowerType.FOSSIL_PRODUCTION)));
      data_p.add(new PowerTypeTemplate(
                                       "Run of river production",
                                       customerTypeSpecific
                                               .get(PowerType.RUN_OF_RIVER_PRODUCTION)));
      data_p.add(new PowerTypeTemplate("Solar production", customerTypeSpecific
              .get(PowerType.SOLAR_PRODUCTION)));
      data_p.add(new PowerTypeTemplate("Wind production", customerTypeSpecific
      ...
      data_s.add(new PowerTypeTemplate("Electric vehicle", customerTypeSpecific
              .get(PowerType.ELECTRIC_VEHICLE)));
      data_s.add(new PowerTypeTemplate(
                                       "Pumped storage production",
                                       customerTypeSpecific
                                               .get(PowerType.PUMPED_STORAGE_PRODUCTION)));

      DrillDownTemplate2 dt2_c =
        new DrillDownTemplate2("Consumers", customerType[0],
                               new DrillDown("Specific", data_c));
      DrillDownTemplate2 dt2_p =
        new DrillDownTemplate2("Producers", customerType[1],
                               new DrillDown("Specific", data_p));
      DrillDownTemplate2 dt2_s =
        new DrillDownTemplate2("Storage", customerType[2],
                               new DrillDown("Specific", data_s));
      ...
      customerStatisticsBroker =
        new DrillDownTemplate(brokerModel.getName(), brokerModel
                .getAppearance().getColorCode(), drillDataBroker);
      ...
      ConcurrentHashMap<Integer, TariffDynamicData> tariffDynData =
        brokerModel.getTariffCategory().getTariffDynamicDataMap();

      Set<Integer> keysTariffDynData =
        new TreeSet<Integer>(brokerModel.getTariffCategory()
                .getTariffDynamicDataMap().keySet()).headSet(safetyTsIndex,
                                                             true);
      ...
      for (Iterator iterator2 = keysTariffDynData.iterator(); iterator2
        ...
        Object[] profit =
          { helper.getMillisForIndex(key), dynData.getDynamicData().getProfit() };
        Object[] netKWh =
          { helper.getMillisForIndex(key), dynData.getDynamicData().getEnergy() };
        ...
        Object[] customerCountOneTimeslot =
          { helper.getMillisForIndex(key), dynData.getCustomerCountDelta() };
      ...
      customerStaticticsArray
              .add(new CustomerStatisticsTemplate(brokerModel.getName(),
                                                  brokerModel.getAppearance()
                                                          .getColorCode(),
                                                  brokerModel
                                                          .getTariffCategory()
                                                          .getCustomerCount(),
                                                  customerStatisticsBroker));//
      for (TariffData data: brokerModel.getTariffCategory().getTariffData()
              .values()) {
        allTarifs.add(data);
      }
    }// end BROKER for loop
