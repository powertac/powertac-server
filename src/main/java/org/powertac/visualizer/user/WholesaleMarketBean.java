...
public class WholesaleMarketBean implements Serializable {
	...
	private void createAllClearings(Gson gson,
		...
		ConcurrentHashMap<Long, Double> totalPriceInTimeslot = new ConcurrentHashMap<Long, Double>();// Tom
		// Number of transactions in timeslot
		ConcurrentHashMap<Long, Integer> numberOfTransactions = new ConcurrentHashMap<Long, Integer>();// Tom
		// Contains total amount of energy traded in each timeslot.
		ConcurrentHashMap<Long, Double> totalEnergyInTimeslot = new ConcurrentHashMap<Long, Double>();// Tom
		...
		for (Iterator iterator = allTrades.iterator(); iterator.hasNext();) {
			...
			for (Iterator iterator2 = trades.iterator(); iterator2.hasNext();) {
				...
				if (!totalPriceInTimeslot.containsKey(helper.getMillisForIndex(ct.getTimeslotIndex()))) {
					...
					totalPriceInTimeslot.put(helper.getMillisForIndex(ct.getTimeslotIndex()), totalPriceInTimeslot.get(helper.getMillisForIndex(ct.getTimeslotIndex()))	+ ct.getExecutionPrice());
					totalEnergyInTimeslot.put(helper.getMillisForIndex(ct.getTimeslotIndex()), totalEnergyInTimeslot.get(helper.getMillisForIndex(ct.getTimeslotIndex()))	+ ct.getExecutionMWh());
					numberOfTransactions.put(helper.getMillisForIndex(ct.getTimeslotIndex()), numberOfTransactions.get(helper.getMillisForIndex(ct.getTimeslotIndex())) + 1);
				}
				// -Tom
			}
		}
		...
		// Tom
		// Sort keys (timeslots)
		SortedSet<Long> sortedSet = new TreeSet<Long>(totalPriceInTimeslot.keySet()).headSet(helper.getMillisForIndex(helper.getSafetyWholesaleTimeslotIndex()), true);
		for (Iterator totalIterator = sortedSet.iterator(); totalIterator.hasNext();) {
			...
			Object[] totalProfitOneTimeslot = { timeslot, totalProfitTimeslot / numberOfTransactions.get(timeslot), totalEnergyInTimeslot.get(timeslot) };
		...
		this.wholesaleAverageTimeslotPriceData = gson.toJson(averageProfitPerTimeslot);// Tom
	...
	private void createMostRecentClearings(Gson gson,
		...
		SortedSet<Long> keys = new TreeSet<Long>(wholesaleService.getClearedTrades().keySet()).headSet(helper.getMillisForIndex(helper.getSafetyWholesaleTimeslotIndex()),true);//mapFinalTrades.keySet());
